...
uint16_t Parser::parse_variable(const std::string& error_message, TokenType type) {
...
void Parser::declaration() {
    if (match(TokenType::TOKEN_CLASS)) { // Consumiu 'class' comeu** hahahahahaha
        // 'current' é o token *seguinte* ao 'class'

        Token class_or_var_name;

        if (check(TokenType::TOKEN_IDENTIFIER)) {
            // Não consome o IDENTIFIER aqui!
            if (check_next(TokenType::TOKEN_LEFT_BRACE)) {
                // Sintaxe: class MyClass { ... }
                // Deixa para class_declaration() consumir o nome da classe
                class_declaration();
            } else {
                // Sintaxe: class myVar; ou class myVar = value;
                advance(); // Consome o IDENTIFIER para a declaração de variável
                Token class_or_var_name = previous;
                declare_variable(class_or_var_name, TokenType::TOKEN_CLASS); // Declara a variável com tipo 'class'
                if (match(TokenType::TOKEN_EQUAL)) { // Se houver um '=' (inicialização)
                    expression(); // Analisa a expressão de inicialização (e.g., `createList()`, `[]`)
                } else {
                    emit_byte(OP_NIL); // Inicializa com nil por padrão se não houver '='
                }
                consume(TokenType::TOKEN_SEMICOLON, "Expect ';' after class variable declaration.");
                define_variable(identifier_constant(class_or_var_name)); // Define a variável globalmente
            }
        } else {
            error_at_current("Expect identifier after 'class' keyword.");
        }
    }
    // Lógica para outros tipos de declaração (import, function, int, bool, etc.)
        ...
        // Se a declaração não começou com 'class', tenta os outros tipos de variáveis
        declaration_statement();
    } else {
        // Se não é uma declaração, tenta como um statement normal (print, if, while, etc.)
        statement();
        // concordo
    }
...
// se você acha que sísifo foi a pessoa que teve mais trabalho para fazer no mundo, imagina eu que tive que mudar todos os tokens para colocar TOKEN_ no início...
...
void Parser::declaration_statement() {
    ...
    consume(TokenType::TOKEN_IDENTIFIER, "Expected variable name after type specifier.");
...
void Parser::function_declaration() {
    ...
    uint16_t global = parse_variable("Expect function name.", TokenType::TOKEN_FUNCTION);
...
void Parser::class_declaration() {
    ...
    declare_variable(class_name, TokenType::TOKEN_CLASS);
    ...
    while (!check(TokenType::TOKEN_RIGHT_BRACE) && !check(TokenType::TOKEN_END_OF_FILE)) {
        if (check(TokenType::TOKEN_FUNCTION)) {
            ...
            klass->methods[method_name.literal] = new_closure(vm, method_body);
...
ObjFunction* Parser::function(TokenType kind, TokenType default_return_type_if_not_explicit) {
    ...
    if (!check(TokenType::TOKEN_RIGHT_PAREN)) {
        do {
            ...
            uint16_t param_const = parse_variable("Expect parameter name.", param_type);
...
TokenType Parser::binary(TokenType left_type, bool can_assign) {
    ...
    if (left_type != TokenType::TOKEN_ILLEGAL && right_type != TokenType::TOKEN_ILLEGAL && !((left_is_numeric && right_is_numeric) || (left_type == TokenType::TOKEN_STRING && right_type == TokenType::TOKEN_STRING))) {
        // SÓ EMITE ERRO SE NÃO ESTIVER EM SOFT MODE
        if (!this->vm->soft_mode) {
    ...
    if (left_type == TokenType::TOKEN_STRING && right_type == TokenType::TOKEN_STRING) {
        if (operator_type == TokenType::TOKEN_PLUS) return TokenType::TOKEN_STRING;
...
TokenType Parser::variable(bool can_assign) {
    ...
    if (can_assign && match(TokenType::TOKEN_EQUAL)) {
        ...
        if (var_type != TokenType::TOKEN_ILLEGAL && !types_are_compatible(var_type, assigned_type)) {
            error("Incompatible types for assignment.");
        }
        if (set_op == OP_SET_LOCAL) {
            emit_bytes(set_op, (uint8_t)arg); // ixi, acho que esqueci de colocar uint16, deixa assim, nínguem vai saber
        } else {
            emit_byte(set_op);
            emit_byte((arg >> 8) & 0xFF);
            emit_byte(arg & 0xFF);
            // eu espero. Será que é melhor colocar uint16? iria expandir o limite. Acho que seria melhor. Meh, deixa assim.
        }
