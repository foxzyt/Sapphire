#ifndef TOPAZ_COMMANDS_SAPPHIRE_CMD_HPP
#define TOPAZ_COMMANDS_SAPPHIRE_CMD_HPP

// =============================================================================
// sapphire_cmd.hpp — Comandos "topaz sapphire *"
//
// Subcomandos disponíveis:
//   topaz sapphire list                   — Lista versões disponíveis no GitHub
//   topaz sapphire versions               — Lista versões instaladas localmente
//   topaz sapphire current                — Mostra a versão ativa
//   topaz sapphire install <ver>          — Instala e ativa uma versão (suporta SemVer)
//   topaz sapphire use <ver>              — Ativa uma versão já instalada
//   topaz sapphire uninstall <ver>        — Remove uma versão instalada
// =============================================================================
...
inline int cmd_sapphire_list() {
    ...
    if (!active.empty()) {
        ...
        std::cout << termcolor::yellow
                  << "Active: (none — run: topaz sapphire install latest)"
                  << termcolor::reset << std::endl;
    }
...
inline int cmd_sapphire_use(const std::string& version) {
    if (version.empty()) {
        std::cerr << termcolor::red
                  ...
                  << termcolor::reset << std::endl;
        std::cerr << "Usage: topaz sapphire use <version>" << std::endl;
...
inline int cmd_sapphire_uninstall(const std::string& version) {
    if (version.empty()) {
        std::cerr << termcolor::red
                  << "[!] Missing version argument"
                  << termcolor::reset << std::endl;
        std::cerr << "Usage: topaz sapphire uninstall <version>" << std::endl;
        return 1;
    }
...
inline void print_sapphire_help() {
    std::cout << termcolor::bold << termcolor::cyan
              << "topaz sapphire" << termcolor::reset
    ...
    std::cout << "  " << termcolor::green << "topaz sapphire versions"
              << termcolor::reset
              << "               List installed versions (local)" << std::endl;
    std::cout << "  " << termcolor::green << "topaz sapphire current"
              << termcolor::reset
    ...
    std::cout << "  " << termcolor::green << "topaz sapphire use <version>"
              << termcolor::reset
              << "          Activate an already-installed version" << std::endl;
    std::cout << "  " << termcolor::green << "topaz sapphire uninstall <version>"
              << termcolor::reset
...
// ---------------------------------------------------------------------------
// cmd_sapphire_dispatch — Ponto de entrada principal para "topaz sapphire ..."
// ---------------------------------------------------------------------------
inline int cmd_sapphire_dispatch(int argc, char* argv[], int sapphire_arg_offset) {
    ...
    // Ex: "topaz sapphire install 1.0.9"  → argv[2]="install", argv[3]="1.0.9"
    ...
    if (subcmd == "list") {
        ...
        std::cerr << termcolor::red
                  ...
                  << termcolor::reset << std::endl;
        std::cerr << "Run 'topaz sapphire help' for usage" << std::endl;
        return 1;
    }
}
...
#endif // TOPAZ_COMMANDS_SAPPHIRE_CMD_HPP
