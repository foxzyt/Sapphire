#ifndef TOPAZ_COMMANDS_TEST_HPP
#define TOPAZ_COMMANDS_TEST_HPP
...
inline int cmd_test(const std::filesystem::path& working_dir, std::string custom_dir = "") {
    ...
    // Parse TopazTestConfig.txt if present
    std::map<std::string, std::string> expected_outputs;
    fs::path config_path = test_dir / "TopazTestConfig.txt";
    ...
    fs::path temp_out = cache_dir / "topaz_test_output.tmp";
