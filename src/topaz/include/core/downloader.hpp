...
inline std::string g_registry_url =
    "https://raw.githubusercontent.com/foxzyt/Sapphire-Topaz/main/";

// Helper: Parse any URL into host and path (supports http and https)
inline bool parse_url(const std::string &url, std::string &host,
                      std::string &path) {
...
inline std::optional<RegistryEntry>
parse_registry_json(const std::string &json_content) {
  try {
...
inline std::optional<RegistryEntry>
query_registry(const std::string &plugin_name) {
  if (g_offline) {
    std::cerr << "[!] Offline mode: Cannot query remote registry for '"
    ...
    if (res && res->status == 200) {
      ...
      if (res->status == 404) {
        std::cerr << "[!] Plugin '" << plugin_name
                  << "' not found in central registry" << std::endl;
        std::cerr << "[!] Available plugins can be found at: "
                     "https://github.com/foxzyt/Sapphire-Topaz"
                  << std::endl;
      } else {
        std::cerr << "[!] HTTP error: " << res->status
                  << " while querying registry" << std::endl;
      }
    } else {
      std::cerr << "[!] Connection failed to registry at: " << host
                << std::endl;
    }
...
inline bool download_file(const std::string &url, const fs::path &output_path) {
  if (g_offline) {
    std::cerr << "[!] Offline mode: Blocked download request for " << url
              << std::endl;
    return false;
  }
  try {
    ...
    auto res = cli.Get(path.c_str(), httplib::Headers(),
        ...
        [&](const char* data, size_t data_length) {
            if (is_200 && out.is_open()) {
                out.write(data, data_length);
            }
            return true;
        },
        [&](uint64_t current, uint64_t total) {
            if (!topaz::g_verbose && is_200) {
                if (total > 0) {
                    ...
                    double kb = current / 1024.0;
                    double total_kb = total / 1024.0;
                    std::cout << "] " << std::fixed << std::setprecision(1) << kb << "/" << total_kb << " kB";
                } else {
                    double kb = current / 1024.0;
                    std::cout << "\r     [========================================] " << std::fixed << std::setprecision(1) << kb << " kB";
                }
                std::cout.flush();
            }
            return true;
    ...
    if (res && (res->status == 301 || res->status == 302 ||
        ...
        if (!new_url.empty()) {
            if (topaz::g_verbose)
                std::cout << "[*] Following redirect to: " << new_url << std::endl;
    ...
    if (res && res->status == 200) {
        if (topaz::g_verbose) {
            std::cout << "[*] Downloaded to " << output_path << std::endl;
        }
        return true;
    } else if (res) {
      if (topaz::g_verbose || res->status != 404) {
        std::cerr << "[!] HTTP error: " << res->status << " (" << res->reason
                  << ")" << std::endl;
      }
    } else {
...
inline std::string github_to_zip_url(const std::string &repo_url,
                                     const std::string &version = "latest") {
  // Expected format: https://github.com/AutorOriginal/economy-sapphire
  // Convert to:
  // https://github.com/AutorOriginal/economy-sapphire/archive/refs/heads/main.zip
  // Or for specific version:
  // https://github.com/AutorOriginal/economy-sapphire/archive/refs/tags/v1.0.0.zip
...
inline bool extract_zip(const fs::path &zip_path, const fs::path &dest_dir) {
  ...
  for (int i = 0; i < file_count; i++) {
    ...
    if (file_stat.m_filename == nullptr ||
        std::strlen(file_stat.m_filename) == 0) {
      continue;
    }
    ...
    if (file_stat.m_is_directory) {
      ...
      // Extract file
      if (!mz_zip_reader_extract_file_to_file(&zip, file_stat.m_filename,
                                              out_path.string().c_str(), 0)) {
        std::cerr << "[!] Failed to extract: " << file_stat.m_filename
                  << std::endl;
        success = false;
      }
    }
...
inline bool
download_and_extract_plugin(const std::string &plugin_name,
  ...
  try {
    ...
    if (fs::exists(zip_path)) {
      if (topaz::g_verbose) {
        ...
        double kb = fs::file_size(zip_path) / 1024.0;
        std::cout << "     Using cached " << cache_key << " (" << std::fixed << std::setprecision(1) << kb << " kB)" << std::endl;
      }
    } else {
      if (g_offline) {
        std::cerr << "[!] Offline mode error: Cache miss for " << plugin_name
                  << " " << version << " (download blocked)" << std::endl;
        return false;
      }
    ...
    if (!expected_checksum.empty()) {
      ...
      if (file.is_open()) {
        ...
        if (actual_checksum != expected_checksum) {
          std::cerr
              << termcolor::red
              << "[!] SECURITY WARNING: Checksum mismatch for downloaded file!"
              << termcolor::reset << std::endl;
          std::cerr << "Expected: " << expected_checksum << std::endl;
          std::cerr << "Got:      " << actual_checksum << std::endl;
          // According to user request, log warning but proceed or reject? The
          // user said "emitindo apenas um alerta caso o pacote nao tenha
          // assinatura... ou se prefere que bloqueie". The user approved the
          // implementation plan. I will block if it HAS a checksum but it
          // MISMATCHES.
          return false;
        } else {
          if (topaz::g_verbose)
            std::cout << termcolor::green << "[+] Checksum verified."
                      << termcolor::reset << std::endl;
        }
      }
#else
      if (topaz::g_verbose)
        std::cout << "[*] Checksum verification skipped (OpenSSL not available)"
                  << std::endl;
#endif
    } else {
      if (topaz::g_verbose)
        std::cout << termcolor::yellow
                  << "[!] WARNING: No checksum provided by registry. Cannot "
                     "verify signature."
                  << termcolor::reset << std::endl;
    }
    ...
    fs::path extract_dir =
        get_cache_dir() / ("extracted_" + plugin_name + "_" + version);
    ...
    // GitHub ZIPs create a root directory (e.g., sapphire-grad-main or
    // sapphire-grad-v1.0.1) We need to find this directory and move its
    // contents
...
inline std::string fetch_latest_version_from_repo(const std::string &repo_url) {
  ...
  if (owner_repo.size() > 4 &&
      owner_repo.substr(owner_repo.size() - 4) == ".git") {
  ...
  try {
    ...
    httplib::Headers headers = {{"User-Agent", "Sapphire-Mine/2.0"},
      ...
      try {
        ...
        if (items.is_array()) {
          for (const auto &item : items) {
            if (item.contains("type") && item["type"] == "dir" &&
                item.contains("name")) {
              ...
              if (highest_version.empty() ||
                  semver::is_older(highest_version, ver)) {
                highest_version = ver;
              }
            }
          }
        ...
        std::cerr << termcolor::yellow
                  << "  [!] Failed to parse versions directory: " << e.what()
                  << termcolor::reset << std::endl;
      }
    }

  } catch (const std::exception &e) {
    std::cerr << termcolor::yellow << "[!] Version fetch error: " << e.what()
              << termcolor::reset << std::endl;
  }
