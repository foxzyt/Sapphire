#ifndef TOPAZ_CORE_SAPPHIRE_VERSION_HPP
#define TOPAZ_CORE_SAPPHIRE_VERSION_HPP
...
inline bool activate_sapphire_version(const std::string& version) {
    ...
    if (!fs::exists(ver_dir)) {
        std::cerr << termcolor::red
                  ...
                  << " is not installed. Run: topaz sapphire install " << clean
                  << termcolor::reset << std::endl;
        return false;
    }
...
#endif // TOPAZ_CORE_SAPPHIRE_VERSION_HPP
