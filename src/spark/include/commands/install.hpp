...
inline int cmd_install_project(bool local_scope = false, bool frozen_lockfile = false, bool no_save = false) {
    ...
    if (!fs::exists(manifest_path)) {
        ...
        std::cerr << "    Run 'spark init' to create one." << std::endl;
        return 1;
    }
    ...
    if (has_lockfile) {
        ...
        try {
            ...
            std::cerr << termcolor::red << "[!] Failed to parse spark.lock: " << e.what() << termcolor::reset << std::endl;
            return 1;
        }
...
inline int cmd_install(const std::string& plugin_name, 
    ...
    // Save to sapphire.json and project-level spark.lock if not no_save
