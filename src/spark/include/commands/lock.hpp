#ifndef SPARK_COMMANDS_LOCK_HPP
#define SPARK_COMMANDS_LOCK_HPP
...
inline int cmd_lock_generate(const std::string& plugin_name, const std::string& version = "latest") {
    ...
    if (!is_plugin_installed_anywhere(plugin_name)) {
        ...
        std::cerr << "Install it first with: " << termcolor::green << "spark install " << plugin_name << termcolor::reset << std::endl;
        return 1;
    }
    ...
    if (fs::exists(lockfile_path)) {
        ...
        std::cout << termcolor::green << "[OK] spark.lock" << termcolor::reset
                  << " (" << size_str << ")" << std::endl;
        std::cout << "  Path: " << termcolor::cyan << lockfile_path.string() << termcolor::reset << std::endl;
    } else {
        std::cout << termcolor::red << "[!] spark.lock was not created" << termcolor::reset << std::endl;
    }
