#ifndef SPARK_COMMANDS_PURGE_HPP
#define SPARK_COMMANDS_PURGE_HPP
...
//
// Usage:
//   spark purge <name>                     — Remove all versions from both scopes
//   spark purge <name> --local             — Remove all versions from local only
//   spark purge <name> --global            — Remove all versions from global only
//   spark purge <name> <version>           — Remove a specific version from both
//   spark purge <name> <version> --local   — Remove a specific version from local
//   spark purge <name> <version> --global  — Remove a specific version from global
// ---------------------------------------------------------------------------
inline int cmd_purge(const std::string& plugin_name, const std::string& version = "",
    ...
    auto remove_version = [&](const fs::path& base_dir, const std::string& scope_name) {
        ...
        try {
            ...
            if (!has_remaining) {
                fs::path lockfile = base_dir / plugin_name / "spark.lock";
...
#endif // SPARK_COMMANDS_PURGE_HPP