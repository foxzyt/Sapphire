...
inline int cmd_init(const std::filesystem::path& working_dir,
    ...
    if (skip_prompts) {
        ...
        std::cout << "=== Spark Project/Plugin Initialization ===" << std::endl;
    ...
    if (p_txt && s_json) {
        ...
        std::cout << termcolor::green << "[+] Use 'spark expand <version>' to add code versioning." << termcolor::reset << std::endl;
        return 0;
    } else {
