#ifndef SPARK_COMMANDS_OUTDATED_HPP
#define SPARK_COMMANDS_OUTDATED_HPP
...
//
// Usage:
//   spark outdated                    — List all outdated plugins
//   spark outdated <name>             — Check a specific plugin
// ---------------------------------------------------------------------------
inline int cmd_outdated(const std::string& plugin_name = "") {
    ...
    if (outdated_count > 0) {
        std::cout << std::endl;
        std::cout << "Run " << termcolor::green << "spark update <name>" << termcolor::reset
...
#endif // SPARK_COMMANDS_OUTDATED_HPP