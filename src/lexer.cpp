...
Token Lexer::string_token() {
    ...
    while (peek() != '"' && !is_at_end()) {
        ...
        if (peek() == '\\') {
            ...
            switch (advance()) { // Consome o caractere após a barra e decide o que fazer
                case 'n':  result += '\n'; break;
                ...
                default:
                    result += '\\';
                    result += source[current - 1]; // Adiciona o caractere desconhecido
                    break;
            }
        } else {
            result += advance(); // Consome e adiciona o caractere normal
        }
    }

    if (is_at_end()) return error_token("Unterminated string.");

    advance(); // Pula o fecha aspas (")
    return make_token(TokenType::TOKEN_STRING_LITERAL, result);
}
