...
private:
    ...
    void add_local(Token name, TokenType type, bool is_const);
    int resolve_local(Compiler* compiler, const Token& name);
    void declare_variable(const Token& name, TokenType type, bool is_const);
    uint16_t parse_variable(const std::string& error_message, TokenType type, bool is_const);
