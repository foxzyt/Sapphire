...
public:
    ...
    // -- Instruções para manipulação de pilha (64-bit) ----------------------
    ...
    // -- Instruções para manipulação de objetos com tamanho variável --------
    ...
    // -- Comparação de string (simplificada) --------------------------------
    void emit_cmp_mem_str_imm(asmjit::x86::Mem mem, const char* str, size_t len) {
        // Não usado no momento - implementação simplificada
        (void)mem; (void)str; (void)len;
    }

    // -- Finalization --------------------------------------------------------

    // Returns: 0 = success, 1 = empty code, 2 = asmjit error
    // If error, the error message is stored in m_lastError
