...
class JitAssembler {
  public: JitAssembler(): m_codeHolder(),
  ...
  void emit_cmp_mem_str_imm(asmjit::x86::Mem mem,
    const char * str, size_t len) {
    // Comparação de string inline (otimizado)
    for (size_t i = 0; i < len; i++) {
      asmjit::x86::Mem byte_mem = mem;
      byte_mem.addOffset(i);
      m_assembler.cmp(byte_mem, (uint8_t) str[i]);
      if (i < len - 1) {
        JitAssembler::Label cont;
        m_assembler.jne(cont);
        // Fallback para comparação de strings mais longas
      }
    }
  }
  ...
  void emit_jcc(CC cc, Label & label) {
    switch (cc) {
    case CC_E:
      m_assembler.je(label);
      break;
    case CC_NE:
      m_assembler.jne(label);
      break;
    case CC_B:
      m_assembler.jb(label);
      break;
    case CC_AE:
      m_assembler.jae(label);
      break;
    case CC_A:
      m_assembler.ja(label);
      break;
    case CC_BE:
      m_assembler.jbe(label);
      break;
    }
  }

  void emit_jz(Label & label) {
    m_assembler.je(label);
  }
  void emit_jnz(Label & label) {
    m_assembler.jne(label);
  }
  void emit_ja(Label & label) {
    m_assembler.ja(label);
  }
  void emit_jb(Label & label) {
    m_assembler.jb(label);
  }
  void emit_jae(Label & label) {
    m_assembler.jae(label);
  }
  void emit_jbe(Label & label) {
    m_assembler.jbe(label);
  }
  ...
  void emit_setcc(CC cc, uint8_t reg) {
    switch (cc) {
    case CC_E:
      m_assembler.sete(asmjit::x86::gpb(reg));
      break;
    case CC_NE:
      m_assembler.setne(asmjit::x86::gpb(reg));
      break;
    case CC_A:
      m_assembler.seta(asmjit::x86::gpb(reg));
      break;
    case CC_B:
      m_assembler.setb(asmjit::x86::gpb(reg));
      break;
    }
  }

  void emit_sete(uint8_t reg) {
    m_assembler.sete(asmjit::x86::gpb(reg));
  }
  void emit_setne(uint8_t reg) {
    m_assembler.setne(asmjit::x86::gpb(reg));
  }
  void emit_seta(uint8_t reg) {
    m_assembler.seta(asmjit::x86::gpb(reg));
  }
  void emit_setb(uint8_t reg) {
    m_assembler.setb(asmjit::x86::gpb(reg));
  }
  ...
  private: asmjit::Environment m_env;
