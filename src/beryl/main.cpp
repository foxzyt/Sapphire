...
int main(int argc, char* argv[]) {
    ...
    // Resolve the true path of beryl.exe itself (works even when called via PATH)
    fs::path build_dir;
#ifdef _WIN32
    ...
    if (argc >= 2 && std::string(argv[1]) == "ui") {
        ...
        if (fs::exists(sapphire_path) && fs::exists(ui_script)) {
            ...
            std::cerr << tc_red() << "Error: Could not find sapphire.exe or beryl_ui.sp in " << build_dir << tc_reset() << "\n";
            return 1;
        }
        return 0;
    }
